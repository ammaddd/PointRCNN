...
class Trainer(object):
    def __init__(self, model, model_fn, optimizer, ckpt_dir, lr_scheduler, bnm_scheduler,
                 ...
                 grad_norm_clip=1.0, comet_logger=None):
        ...
        with tqdm.trange(start_epoch, n_epochs, desc='epochs') as tbar, \
            ...
            for epoch in tbar:
                ...
                if self.bnm_scheduler is not None:
                    ...
                    self._comet_logger.log_metric('bn_momentum',self.
                ...
                for cur_it, batch in enumerate(train_loader):
                    if lr_scheduler_each_iter:
                        ...
                        self._comet_logger.log_metric('learning_rate',
                                                      cur_lr, step=it,
                                                      epoch=epoch)
                    else:
                    ...
                    if self.tb_log is not None:
                        ...
                        self.tb_log.add_scalar('learning_rate', cur_lr, it)
                        self._comet_logger.log_metric('train_loss', loss,
                                                      step=it, epoch=epoch)
                        self._comet_logger.log_metric('learning_rate',
                                                      cur_lr, step=it,
                                                      epoch=epoch)
                        for key, val in tb_dict.items():
                            ...
                            self._comet_logger.log_metric('train_' + key,
                                                          val, step=it,
                                                          epoch=epoch)
                ...
                if trained_epoch % ckpt_save_interval == 0:
                    ...
                    self._comet_logger.log_model("PointRCNN", '{}.pth'.format(ckpt_name))
                ...
                if (epoch % eval_frequency) == 0:
                    ...
                    if test_loader is not None:
                        ...
                        if self.tb_log is not None:
                            ...
                            self._comet_logger.log_metric('val_loss',
                                                          val_loss,
                                                          step=it,
                                                          epoch=epoch)
                            for key, val in eval_dict.items():
                                self.tb_log.add_scalar('val_' + key, val, it)
                                self._comet_logger.log_metric('val_' + key,
                                                              val, step=it,
                                                              epoch=epoch)
